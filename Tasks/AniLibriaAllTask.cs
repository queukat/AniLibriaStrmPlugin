...
public sealed class AniLibriaAllTask : IScheduledTask
{
    ...
    public async Task ExecuteAsync(IProgress<double> progress, CancellationToken token)
    {
        ...
        try
        {
            ...
            logBuf.AppendLine("Fetching title list …");
            var titles = await _client.FetchAllTitlesAsync(cfg.AllTitlesPageSize, cfg.AllTitlesMaxPages, token);
            logBuf.AppendLine($"Titles fetched: {titles.Count}");

            await _gen.GenerateTitlesAsync(titles, cfg.StrmAllPath, cfg.PreferredResolution, progress, token);
        }
        catch (Exception ex)
        {
